...
class PS:
    ...
    def get_student_count(self, school_id):
        ...
        response = self.standard_access.fetch_data(
            endpoint, headers=json_headers)
    ...
    def get_students(self, school_id, pagesize=100):
        ...
        # Ceiling division to get total pages
        ...
        for page in range(1, total_pages + 1):
            ...
            response = self.standard_access.fetch_data(
                endpoint, params=params, headers=json_headers)
    ...
    def get_reenroll_parents(self, student_id):
        endpoint = f"ws/v1/student/{
            student_id}?expansions=school_enrollment&extensions=u_re_enrollment_extension"
        response = self.standard_access.fetch_data(
            endpoint, headers=json_headers)

        # Extracting only the relevant fields
        reenroll_info = response.get('student', {}).get(
            '_extension_data', {}).get('_table_extension', {})
        parent_fields = {
    ...
    def get_students_in_grade(self, school_id, grade, pagesize=100):
        ...
        students_by_grade = {}
        for page in range(1, total_pages + 1):
            endpoint = f"ws/v1/school/{
                school_id}/student?expansions=school_enrollment&q=school_enrollment.enroll_status_code==0"
            params = {'page': page, 'pagesize': pagesize}
            data = self.standard_access.fetch_data(
                endpoint, params=params, headers=json_headers)
